...
mutual
partial def expandLeafnyDoSeq (modIds : Array Ident) (stx : doSeq) (insideLoop: Bool := false) : TermElabM (Array doSeqItem) :=
  match stx with
  | `(doSeq| $doS:doSeqItem*) => return Array.flatten $ ← doS.mapM (expandLeafnyDoSeqItem modIds (insideLoop:=insideLoop))
  | _ => throwErrorAt stx s!"unexpected syntax of Velvet `do`-notation sequence {stx}"


partial def expandLeafnyDoSeqItem (modIds : Array Ident) (stx : doSeqItem) (insideLoop: Bool := false) : TermElabM (Array doSeqItem) := do
  match stx with
  -- Ignore semicolons
  | `(Term.doSeqItem| $stx ;) => expandLeafnyDoSeqItem (insideLoop:=insideLoop) modIds $ <- `(Term.doSeqItem| $stx:doElem)
  | `(Term.doSeqItem| return) =>
    if insideLoop then
      throwErrorAt stx s!"return not allowed from a loop, modify your program to use break instead"
    expandLeafnyDoSeqItem (insideLoop:=insideLoop) modIds $ <- `(Term.doSeqItem| return ())
  ...
  | `(Term.doSeqItem| if $h:ident : $t:term then $thn:doSeq else $els:doSeq) =>
    let thn <- expandLeafnyDoSeq modIds thn (insideLoop:=insideLoop)
    let els <- expandLeafnyDoSeq modIds els (insideLoop:=insideLoop)
    let ret ← `(Term.doSeqItem| if $h:ident : $t then $thn* else $els*)
    return #[ret]
  | `(Term.doSeqItem| if $h:ident : $t:term then $thn:doSeq) =>
    if insideLoop then
      let thn <- expandLeafnyDoSeq (insideLoop:=insideLoop) modIds thn
      let ret ← `(Term.doSeqItem| if $h:ident : $t then $thn*)
      return #[ret]
    else
      expandLeafnyDoSeqItem (insideLoop:=insideLoop) modIds $ <- `(Term.doSeqItem| if $h:ident : $t:term then $thn else pure ())
  | `(Term.doSeqItem| if $t:term then $thn:doSeq) =>
    if insideLoop then
      let thn <- expandLeafnyDoSeq (insideLoop:=insideLoop) modIds thn
      let ret ← `(Term.doSeqItem| if $t then $thn*)
      return #[ret]
    else
      expandLeafnyDoSeqItem (insideLoop:=insideLoop) modIds $ <- `(Term.doSeqItem| if $t then $thn else pure ())
  | `(Term.doSeqItem| match $discrs:matchDiscr,* with $[$alts:matchAlt]*) =>
    let alts' ← alts.mapM fun (alt : TSyntax _) => do
      ...
      | `(doMatchAltExpr| | $a,* => $rhs:doSeq) =>
        let rhs' ← expandLeafnyDoSeq modIds rhs (insideLoop:=insideLoop)
  ...
  | `(Term.doSeqItem| while $t:term
      ...
      do $doSeq:doSeq) =>
    let doSeq <- expandLeafnyDoSeq modIds doSeq (insideLoop:=true)
    return #[<-`(Term.doSeqItem| while $t:term
      $[invariant $[$invName:str]? $inv:term
      ]*
      $[done_with $inv_done]?
      $[decreasing $measure]?
...
/-- Names of gadget functions whose arguments should be excluded from forbidden constant checking -/
private def specGadgetNames : Array Name := #[
  `invariantGadget,
  `onDoneGadget,
  `assertGadget,
  `decreasingGadget
...
/-- Check if an expression is an application of one of the spec gadgets -/
private def isSpecGadgetApp (e : Expr) : Bool :=
  let fn := e.getAppFn
  match fn with
  | .const n _ => specGadgetNames.contains n
  | _ => false

/-- Find forbidden constants in an expression, excluding expressions that are
    arguments to spec gadgets (invariantGadget, onDoneGadget, etc.) -/
private partial def findForbiddenConstsExcludingGadgets (e : Expr) (forbidden : Array Name) : NameSet :=
  go e {}
where
  go (e : Expr) (acc : NameSet) : NameSet :=
    -- Skip if this is a spec gadget application
    if isSpecGadgetApp e then acc
    else match e with
      | .const n _ => if forbidden.contains n then acc.insert n else acc
      | .app f arg => go arg (go f acc)
      | .lam _ ty body _ => go body (go ty acc)
      | .forallE _ ty body _ => go body (go ty acc)
      | .letE _ ty val body _ => go body (go val (go ty acc))
      | .mdata _ e' => go e' acc
      | .proj _ _ e' => go e' acc
      | _ => acc
